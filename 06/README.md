...
In these notes we'll continue on from last time, and talk about what happens to objects when things go wrong, and how this influences many parts of C++.
...
Programmers often ignore these return codes, and if they didn't code would be full of if statements and be much harder to read.
...
This `catch` clause is an *exception handler*, which should handle the error that caused the exception.
If there is no appropriate `catch`, then the stack continues to be unwound until an appropriate `catch` is found, or if we reach `main`, in which case the program will crash.

INSERT LINK exceptions.cpp
...
Inside the catch clause, you can rethrow an exception (or a different object) to restart the process, unwinding the stack at the current spot again to find another suitable exception handler.
You can also use `throw;` to rethrow the caught exception (useful for the `catch(...)` case, where the exception itself is unnamed).
...
One very important invariant in C++ is that whenever an object is destroyed, its destructor will be called.
This includes when the stack is unwound due to an exception, so destructors can be relied upon to always be called.
...
One classic solution to this, resulting in a copy that has a strong guarantee, is called *copy and swap*.
...
INSERT LINK swap1.cpp

We use `std::swap` to swap members of the two objects, (found in the `<utlity>` header, and some other standard headers) which is `noexcept`.
...
You may be wondering if we need to swap the members of our objects, rather than just swapping the objects themselves (like `swap(*this, i)`).
...
In the code above, we define the `friend` function in the class itself, which is a shortcut since I wrote the code in a single file.
This is equivalent to declaring it as a `friend` and defining it outside of the class.

## RAII: Resource Acquisition Is Initialization

RAII is a technique in C++ where resources are bound to the lifetimes of objects.

This guarantees that once a resource is acquired via the constructor, it is guaranteed to be released via the destructor.
Instead of using functions like `open()` and `close()`, the resource is instead tied to the object creation and destruction, allowing us to leverage the guarantees that C++ gives us about objects, and avoiding resource leaks.
...
For example, the standard library provides a [`mutex`](https://en.cppreference.com/w/cpp/thread/mutex) class for mutual exclusion to be used for concurrency.
...
A `unique_ptr` deletes its memory in its destructor, and these assumptions help guarantee that this operation is safe, that nobody else is holding a pointer to this same memory.
...
To initialize a `unique_ptr`, it can be initialized directly with the address it is meant to manage, as `unique_ptr<int> i {new int {1}};`.
Alternatively, we can use `auto` and `make_unique`, as `auto i {make_unique<int>(1)};`.
This second option is recommended, as it allows us to avoid repeating the type twice (annoying for long type names).
It also calls the constructor of the newly allocated object for us, based on the arguments we pass, so we don't need to call `new` ourselves.
This also ensures that the pointer that the `unique_ptr` manages is really something that is owned solely by it, instead of just any random pointer, which could be the case if we initialize it in the first way.
...
Be sure not to just storing and using this, however, since that would completely defeat the purpose of using a smart pointer.
- `reset(p)` which deletes the current pointer, and replaces it with a new pointer to manage, `p`.
- `release()` which returns the underlying raw pointer, and releases it from being managed by this `unique_ptr`. This does not delete the memory, and basically gives up ownership of the memory to the caller.
...
Internally, `shared_ptr`s keep track of the number of `shared_ptr`s pointing to the same memory, and will only `delete` the pointer if they are the last one remaining.
...
Doing the latter will lead to the two `shared_ptr`s not knowing about each other, each thinking they are the sole owner of the memory, leading to a double free and undefined behavior.
Instead, to have multiple owners of the same memory, create a `shared_ptr` and copy it using either the copy constructor or the assignment operator.
This will properly use the internal systems of the `shared_ptr` to increment the reference count properly.

INSERT LINK shared.cpp
...
If that `shared_ptr` is still alive, then `lock()` returns a copied `shared_ptr`, allowing us to use it to do standard pointer operations.
...
Otherwise, if the original `shared_ptr` no longer holds that memory, then `lock()` returns a null `shared_ptr`, signifying that this `weak_ptr` cannot access the memory anymore.

INSERT LINK weak.cpp
...
Finally, note that the code above is not that good---it requires us to know the initialization of `q` is the one that fails, so we `delete` the memory that we know was successfully allocated: `p`.
